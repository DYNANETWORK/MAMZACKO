...
# Made by @SUKHPAL_KHERERA

# Edit anything at your own risk
from MAMZACKO import Image
import os
...
async def start_t(bot, message):
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('Updates Channel', url='https://t.me/MAMBA_NETWORK'), InlineKeyboardButton('Support Group', url='https://t.me/MAMBA_X_SUPPORT')]])
    )
...
async def telegraph(bot, message):
    ...
    if message.sticker:
        ...
        img.save(f'{DOWNLOAD_DIRECTORY}/SUKHPAL_KHERERA-{message.from_user.id}.png', 'png')
        os.remove(down_loc)
        down_loc = f'{DOWNLOAD_DIRECTORY}/MAMZACKO)STAR-{message.from_user.id}.png'
